#!/usr/bin/env python3
"""
Genera light_mode.svg i dark_mode.svg: una targeta de terminal (sense marc de finestra) amb les
...
CONFIG = {
    ...
    "ascii_cols": 44,
    "ascii_invert": True,                       # intercanvia @ i espai al retrat
    "headline": [                               # text animat de la capçalera (header.svg)
        "Bones! Sóc l'Oriac 👋",
        "Backend de CityXerpa",
        "Ping-pong master 🏓",
    ],
...
THEMES = {
    "light_mode.svg": {
        ...
        "accent": "#e0484a", "key": "#e0484a", "art": "#ff5657",
    },
    "dark_mode.svg": {
        "bg": "#0d1117", "panel": "#151b23", "border": "#30363d",
        "fg": "#c9d1d9", "dim": "#8b949e",
        "accent": "#ff5657", "key": "#ff5657", "art": "#ff5657",
    },
}
...
def fetch_loc(s, repos):
    ...
    for repo in repos:
        ...
        if resolved:                      # un 202 esgotat no es guarda: es reintenta demà
...
def invert(lines):
    """On hi havia @ posa espai i on hi havia espai posa @."""
    width = max(len(l) for l in lines)
    swap = str.maketrans("@ ", " @")
    return [l.ljust(width).translate(swap).rstrip() for l in lines]


def load_ascii():
    handmade = ROOT / CONFIG.get("ascii_file", "")
    if CONFIG.get("ascii_file") and handmade.exists():
        art = crop_frame(handmade.read_text(encoding="utf-8").splitlines())
    else:
        art = to_ascii(CONFIG["avatar"], CONFIG["ascii_cols"])
    return invert(art) if CONFIG.get("ascii_invert") else art
...
HEAD_W, HEAD_H, HEAD_SIZE = 700, 58, 26


def build_header(lines):
    """Text centrat que s'escriu lletra a lletra, una línia darrere l'altra, en bucle."""
    n = len(lines)
    slot = 100 / n
    type_share = 0.62                     # part del torn dedicada a escriure
    total = 4.4 * n                       # segons de tota la volta
    o = [f'<svg xmlns="http://www.w3.org/2000/svg" width="{HEAD_W}" height="{HEAD_H}" '
         f'viewBox="0 0 {HEAD_W} {HEAD_H}" font-family="{FONT}" role="img" '
         f'aria-label="{esc(" · ".join(lines))}">']
    css = ["text { white-space: pre; dominant-baseline: middle; "
           "clip-path: inset(0 100% 0 0); }"]
    for i, line in enumerate(lines):
        a, b, c = i * slot, (i + type_share) * slot, (i + 1) * slot
        chars = max(1, len(line))
        css.append(
            f"@keyframes l{i} {{ 0%,{a:.2f}% {{ clip-path: inset(0 100% 0 0) }} "
            f"{b:.2f}%,{c - 0.01:.2f}% {{ clip-path: inset(0 0 0 0) }} "
            f"{c:.2f}%,100% {{ clip-path: inset(0 100% 0 0) }} }}")
        css.append(f".l{i} {{ animation: l{i} {total}s steps({chars}) infinite; }}")
    css.append("@media (prefers-reduced-motion: reduce) { text { animation: none; "
               "clip-path: none } text:not(.l0) { display: none } }")
    o.append("<style>" + "\n".join(css) + "</style>")
    for i, line in enumerate(lines):
        o.append(f'<text class="l{i}" x="{HEAD_W/2}" y="{HEAD_H/2}" text-anchor="middle" '
                 f'font-size="{HEAD_SIZE}" font-weight="500" fill="{HEAD_COLOR}">'
                 f'{esc(line)}</text>')
    o.append("</svg>")
    return "\n".join(o)
...
def main():
    ...
    art = load_ascii()
    lines = compose(stats)
    title = f"{CONFIG['user']} — perfil"

    for filename, theme in THEMES.items():
        (ROOT / filename).write_text(build_svg(theme, art, lines, title), encoding="utf-8")
        print("escrit", filename)
    (ROOT / "header.svg").write_text(build_header(CONFIG["headline"]), encoding="utf-8")
    print("escrit header.svg")
